...
import {
  ...
  ConsoleGenre,
  useConsoleContext,
} from "./ConsoleGameManagerContext";
...
export const GamePopup: React.FC<GamePopupProps> = ({
  ...
  useEffect(() => {
    if (gameToEdit) {
      ...
      setFormData({
        ...
        description: "",
        releaseDate: "",
      }); // Default releaseDate removed for clarity
      setSelectedConsoles([]);
      setSelectedGenreIds([]);
      setNewGenreInput("");
      setSearchResults([]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const newGenreNames = newGenreInput
        .split(",")
        .map((name) => name.trim())
    ...
    try {
      ...
      if (searchSource === "steam") {
        response = await fetch(
          `/api/consoles/steam/search?name=${encodeURIComponent(searchQuery)}`,
        );
      } else {
        response = await fetch(
          `/api/consoles/vgg/search?gameName=${encodeURIComponent(searchQuery)}`,
        );
      }
      if (!response.ok)
  ...
  const handleSelectResult = async (result: SearchResult) => {
    ...
    try {
      ...
      if (!response.ok)
        throw new Error(
          `Fetching details failed with status: ${response.status}`,
        );
      ...
      setFormData((prev) => ({
        ...
        boxImageUrl:
          (searchSource === "steam" ? data.header_image : data.imageurl) ||
          prev.boxImageUrl, // Correct VGG field might be imageurl
        description:
          (searchSource === "steam"
        ...
        releaseDate:
          (searchSource === "steam"
            ...
            : data.yearpublished?.toString()) || prev.releaseDate, // Ensure VGG year is string
      }));
      ...
      if (
        ...
        const steamGenreNames = data.genres.map((g: SteamGenre) =>
          g.description.trim(),
        );
        // Ensure allGenres is an array before mapping
        const safeAllGenres = Array.isArray(allGenres) ? allGenres : [];
        const existingGenreMap = new Map<string, number>(
          safeAllGenres.map((g: ConsoleGenre) => [g.name.toLowerCase(), g.id]),
        );
        ...
        steamGenreNames.forEach((name: string) => {
          ...
          if (existingId !== undefined) {
            ...
            if (
              !unmatchedGenreNames.some(
                (un) => un.toLowerCase() === lowerCaseName,
              )
            ) {
  ...
  const safeAllGenres: ConsoleGenre[] = Array.isArray(allGenres)
    ? allGenres
    : [];
      ...
      <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        {showSearch ? (
          ...
          <form onSubmit={handleSubmit} className="space-y-4 p-1">
            ...
            <div className="grid grid-cols-4 items-center gap-4">
              {" "}
              {/* Changed gap for alignment */}
              <Label htmlFor="game-release" className="text-right">
                Release Info
              </Label>{" "}
              {/* Changed label */}
              <Input
                ...
                onChange={handleInputChange}
                className="col-span-3"
                placeholder="e.g., 2023 Q4, Jan 15, 2024"
              />{" "}
              {/* Changed placeholder */}
            </div>
            ...
            {/* New Genre Input */}
            <div className="grid grid-cols-4 items-center gap-4">
              <Label htmlFor="new-genres" className="text-right">
                Add New Genres
              </Label>
              <Input
                id="new-genres"
            ...
            <div className="grid grid-cols-4 gap-4">
              ...
              <div className="col-span-3 flex flex-col gap-2">
                {safeConsoles.length === 0 ? (
                  <div className="text-sm text-gray-500 italic">
                    No consoles available. Add one first.
                  </div>
                ) : (
                  safeConsoles.map(
                    (
                      console: Console, // Added Console type
                    ) => (
                      <div
                        key={console.id}
                      ...
                      >
                        <Checkbox
                          ...
                          onCheckedChange={() =>
                            handleConsoleToggle(console.id!)
                          }
                        />
                        <Label
